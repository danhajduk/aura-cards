...
class AuraWeatherCard extends HTMLElement {
    ...
    setConfig(config) {
      ...
      const iconSet = config.icon_set || 'basmilius'; // default to basmilius
    ...
    _getWeatherIcon(condition) {
      ...
      return fileName ? `/local/community/aura-cards/icons/${fileName}` : '';
    }
    ...
    _updateContent() {
      ...
      el.innerHTML = `
        ...
        ${wind ? ` | 💨 ${wind}` : ''}
        ${uv ? ` | ☀️ <span class="${color(uv, [3, 6])}">UV ${uv}</span>` : ''}
        ${rain ? ` | 🌧️ ${rain}%` : ''}
        ${storm ? ` | ⚡ ${storm}%` : ''}
